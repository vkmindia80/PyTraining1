...
function called __init__(), which allows you to do some initializing when the object
is being created.
...
More Examples
In the below example, we have a Cars class, which is both iterable and iterator. The Cars
class is an iterator because it implements ?
...
# return the next car name
        ...
        self.__index += 1
        return car
#Using above code create an iterator
myclass = Cars ()
myiter = iter(myclass)
for x in myiter:
    print(x)
